...
pour travailler sur des ronds proportionnels,il faut d'abord créer le centroide de nos zones et ensuite tracer un rond proportionnel avec geom_sf.
...
tt<-World %>%
  ...
  inner_join(indicateur311 %>%
              ...
              filter(!is.na(Value)) %>%
              filter(Year==max(Year)))
ttc<-st_centroid(tt, st_centroid(of_largest_polygon=TRUE))
...
*Attention: les centroides ne sont pas forcément là où on le pense :
par exemple, sans l'option 'of_largest_polygon=TRUE' le centroide de la France serait décalé en Espagne puisqu'il prend en compte les DOM.*
